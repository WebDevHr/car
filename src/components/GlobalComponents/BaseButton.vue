<template>
    <component :is="tag" :class="buttonClasses" :type="nativeType" v-bind="$attrs" @click="$emit('click', $event)">
        <span class="v-btn__slot">
            <slot />
        </span>
    </component>
</template>
...
@each $type-name,
$theme-map in $button-type-theme-maps {
    $all: ();
